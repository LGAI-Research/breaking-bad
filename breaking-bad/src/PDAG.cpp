...
void PDAG::apply_delete_raw(const Delete &delet,
                        EdgeModificationsMap &edge_modifications_map) {
    auto start_time = high_resolution_clock::now();
    if (has_directed_edge(delet.x, delet.y)) {
        // 1. remove the directed edge x → y
...
void PDAG::remove_edge_only_no_finalize(int x, int y, bool directed_x_to_y,
    EdgeModificationsMap &em,
    EdgeQueueSet &edges_to_check_acc) {

if (directed_x_to_y) {
    if (!has_directed_edge(x, y)) return;
    remove_directed_edge(x, y); 
    em.update_edge_none(x, y, EdgeType::DIRECTED_TO_Y);
} 
else {
    if (!has_undirected_edge(x, y)) return;
    remove_undirected_edge(x, y);
    em.update_edge_none(x, y, EdgeType::UNDIRECTED);
}
add_adjacent_edges(x, y, edges_to_check_acc);
}
...
void PDAG::complete_cpdag_global(EdgeModificationsMap &edge_modifications_map) {
...
PDAG PDAG::from_file_pdag(const std::string &filename) {
    std::ifstream file(filename);
    std::string line;
    std::vector<int> nodes;
    ...
    for (int i =0; i<nodes.size(); i++){
        for (int j = i+1 ; j<nodes.size(); j++){ // check only upper triangular matrix
            if (M[i][j] == 1 && M[j][i] == 1){
                graph.add_undirected_edge(nodes[i], nodes[j]);
                ...
                throw std::runtime_error("Invalid matrix M");
            }
        }
    }
    return graph;
